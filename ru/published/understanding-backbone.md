...
[Backbone.js](http://backbonejs.org/). В этом посте я буду постепенно
...
текст, отправить его, и отображает ваш текст в списке, пока текстовое
...
HTML-шаблонизацию. Все это в 16 строчках кода. Похоже, что
я написал этот JavaScript год назад. Походу поста я буду постепенно
приводить код к моему стилю, следующий принципу единой ответственности
(single responsibility principle) и становящийся проще для тестирования,
сопровождения и расширения.

Вот три вещи, которые мы хотим достигнуть:
...
Для решения этой проблемы мы можем использовать
[`$.proxy`](http://api.jquery.com/jQuery.proxy/), создающий функцию, где
`this` всегда тоже, что и контекст, который вы указываете вторым
аргументом.
...
Давайте вместо работы непосредственно с DOM сделаем callback методы
